...
export const seedDatabase = asyncHandler(async (req, res) => {
  try {
    // Check if seeding is allowed (only allow in development or with a secret key)
    const seedKey = req.headers["x-seed-key"];
    const allowedKey = process.env.SEED_KEY || "dev-seed-key";

    if (process.env.NODE_ENV === "production" && seedKey !== allowedKey) {
      res.status(403);
      throw new Error("Seeding not allowed in production without valid seed key");
    }
